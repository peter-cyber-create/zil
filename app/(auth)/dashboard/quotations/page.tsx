import { getAllQuotations } from "@/app/actions/dashboard";
import RespondForm from "@/components/RespondForm";
import Link from "next/link";
import { FaFileInvoice, FaTachometerAlt } from "react-icons/fa";
import LogoutButton from "@/components/LogoutButton";
import { Quotation } from "@/types/dashboard";
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#232526] text-white">
      ...
      <main className="w-full md:max-w-5xl md:mx-auto px-0 md:px-6 mt-10">
        <h1 className="text-2xl md:text-3xl font-extrabold tracking-tight flex items-center gap-3 text-[#00c6ff] drop-shadow mb-8 pl-2 md:pl-0">
          <FaFileInvoice className="text-[#00c6ff]" /> Quotation Requests
        </h1>
        <ul className="space-y-4 md:space-y-6 overflow-y-auto max-h-[70vh] pr-0 md:pr-2 w-full">
          {quotations.length === 0 && <li className="text-gray-400 text-lg pl-2 md:pl-0">No quotations yet.</li>}
          {quotations.map((q: Quotation) => (
            <li key={q.id} className="w-full bg-[#232526] rounded-none md:rounded-xl shadow p-2 md:p-6 flex flex-col border border-[#00c6ff]/10">
              <div className="flex flex-col md:flex-row md:items-center gap-1 md:gap-2 font-bold text-white text-base md:text-lg">
                <span>{q.name}</span>
                <span className="text-xs md:text-sm text-gray-300">({q.email})</span>
                {q.phone && <span className="text-xs md:text-sm">- {q.phone}</span>}
                {q.responded ? <span className="text-green-400 ml-0 md:ml-1">Responded</span> : <span className="text-yellow-400 ml-0 md:ml-1">Awaiting response</span>}
              </div>
              <div className="text-sm md:text-base text-gray-200 mb-2">{q.details}</div>
              {q.responded ? (
                <div className="text-green-400 text-xs md:text-sm">Responded: {q.response}</div>
              ) : (
